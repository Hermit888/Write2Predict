...
but merges some confused letters (e.g. `C` and `c`) to 47 classes. All images are in a 28x28 pixel grayscale format.

# Train the Model
All codes and results are in `AI Training Model/Training Model.ipynb` and were written by Pytorch following the below steps:

### 1. Get and Preprocess Dataset
Load training data and test data from the EMNIST dataset. Rotate the image by 90°, then flip horizontally to makethe  pictures' orientation consistent with how people normally view images. The tensor vectors are also normalized to the range of [-1, 1]. The batch size set 64.

### 2. Build the Model
The forward of the convolutional neural network (CNN) is as follows: <br>
```
input
...
Dropout prevents overfitting by randomly deactivating neurons during training. The first dropout in the convolution layer is the probability of 25%, and the second one in the linear layer is the probability of 50%.

### 3. Optimization
Each parameter is respectively learning rate = 1e-3 and weight decay(L2 regularization) = 1e-5. There is also a scheduler to monitor the loss and reduce the learning rate if the loss does not decrease in 2 epochs.

### 4. Learning Epochs
The model runs 20 epochs and records each training loss, training accuracy, validation (test) loss, and validation accuracy. In each training iteration, the model is updated through a backpropagation function.
...
# Website
### Description
The web contains a canvas that allows the user to draw a character on it. After that, users can press `predict` button to get the predicted result of the CNN model. If there is nothing on canvas, the error will pop up.<br>
